            ...
            <div className="w-full max-w-xl space-y-8 z-10">
                ...
                <div className="flex justify-center mb-4">
                    <motion.div initial={{ rotate: -10 }} animate={{ rotate: 0 }} className="flex items-center gap-2">
                        ...
                        <span className="font-serif italic text-2xl tracking-tighter text-white">Escapade</span>
                    </motion.div>
                </div>
                    ...
                    {[1, 2, 3].map((s) => (
                        <div key={s} className="flex items-center gap-4 flex-1 last:flex-none">
                            <div className={cn(
                                ...
                                step >= s ? "bg-primary border-primary text-white" : "bg-white/5 border-white/10 text-white/40"
                            )}>
                                {step > s ? <Check className="w-4 h-4" /> : s}
                            </div>
                            {s < 3 && <div className={cn("flex-1 h-px", step > s ? "bg-primary" : "bg-white/10")} />}
                        </div>
                    ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <Card className="glass border-white/5 shadow-2xl overflow-hidden min-h-[450px] flex flex-col">
                            {step === 1 && (
                                <div className="p-8 space-y-8 flex-1">
                                    <div className="space-y-2">
                                        <CardTitle className="text-3xl font-serif italic text-white">Choose your vibe.</CardTitle>
                                        <CardDescription className="text-white/40 text-base">Select the atmospheres that define your weekend energy.</CardDescription>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        {VIBE_OPTIONS.map((v) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <Icon className={cn("w-6 h-6 mb-3 transition-colors", isSelected ? "text-primary" : "text-white/40 group-hover:text-white/60")} />
                                                    <div className="font-bold text-white mb-1">{v.label}</div>
                                                    <div className="text-[10px] text-white/30 uppercase tracking-widest font-bold">{v.desc}</div>
                                                </button>
                                            )
                                        })}
                                    </div>
                            ...
                            {step === 2 && (
                                <div className="p-8 space-y-8 flex-1">
                                    <div className="space-y-2">
                                        <CardTitle className="text-3xl font-serif italic text-white">Budget Persona.</CardTitle>
                                        <CardDescription className="text-white/40 text-base">How do you like to allocate your escapade funds?</CardDescription>
                                    </div>
                                    <div className="space-y-3">
                                        {BUDGET_PERSONAS.map((p) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center", isSelected ? "bg-primary text-white" : "bg-white/10 text-white/40")}>
                            ...
                            {step === 3 && (
                                <div className="p-8 space-y-8 flex-1">
                                    <div className="space-y-2">
                                        <CardTitle className="text-3xl font-serif italic text-white">Interest Tags.</CardTitle>
                                        <CardDescription className="text-white/40 text-base">Select your core interests for personalized discovery.</CardDescription>
                                    </div>
                            ...
                            <div className="p-8 pt-0 flex justify-between items-center bg-gradient-to-t from-black/20 to-transparent">
                                <Button
                                    variant="ghost"
                                    onClick={() => step > 1 && setStep(step - 1)}
                                    className={cn("text-white/40 hover:text-white", step === 1 && "opacity-0 pointer-events-none")}
                                >
                                    Previous
                                </Button>
                                <Button
