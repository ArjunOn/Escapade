...
import { Compass, Mail, Lock, AlertCircle, X, Chrome } from "lucide-react";
...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      // Small delay for store to hydrate from auth state change
      setTimeout(() => {
  ...
  const handleGoogle = async () => {
    ...
    catch (err: any) { setError(err.message || "Google sign-in failed"); setOauthLoading(null); }
  };

  const handleFacebook = async () => {
    setError(""); setOauthLoading("facebook");
    try { await signInWithFacebook(); onClose(); }
    catch (err: any) { setError(err.message || "Facebook sign-in failed"); setOauthLoading(null); }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-center text-sm text-[var(--color-text-secondary)]">
            No account?{" "}
            <Link href="/signup" onClick={onClose} className="text-[var(--color-primary)] font-medium hover:underline">Sign up free</Link>
          </p>
        </div>
      </div>
    </div>
